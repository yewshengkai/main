...
The find mechanism is facilitated by the `FindCommand`. `FindCommandParser` utilizes `ArgumentTokenizer` to tokenize +
...
utilizes `arePrefixesPresent` method to retrieve the string which utilizes `ParserUtil` to check the validity of string +
expression.
...
if-else statement to check if any prefixes are present when user execute the find operation. +
...
`MainWindow` code snippet to give/set ThemeCommand `Region`:
[source,java]
