...
`FindCommandParser` code snippet to determine if user's command contains any parameter:
[source,java]
----
...
through `ParserUtil` and calls the relevant `ThemeCommand` to change the application `theme`.
...
[NOTE]
When the application is first initialize, 
