...
// tag::undoredo[]
=== Find/Findany  mechanism

The find/findany mechanism resides inside `PersonContainsKeywordsPredicate`. It supports find and findany with any prefixes (`noprefix/NAME, a/ADDRESS, e/EMAIL, h/HOMEPAGE, p/PHONE, t/TAGS`), that generate a list of person with details from the address book (e.g. `find john`, `find p/91234567`). Such commands will be called from `FindCommand` and `FindCommandParser`.

`PersonContainsKeywordsPredicate` deals with `FindCommand` and 'FindCommandParser'. The following diagram shows the inheritance diagram for commands:
