...
*Alternative 1 (Current choice):* Enable `MainWindow` at start to set/give ThemeCommand its `Region` before utilizing setTheme method to switch stylesheet. +
*Pros:* Prevent unnecessary re-access to retrieve application `Region` or access to other `class` file. Check if file exist and clear current stylesheet before changing the new style. +
*Cons:* There are many other ways to change the styling of application (e.g. Utilizing `ChangeThemeRequestEvent` and call methods in `MainWindow`) +
*Alternative 2:* Utilizing `ChangeThemeRequestEvent` and allow `MainWindow` to handle the RequestEvent and call method to change theme. +
*Pros:* ThemeCommand will have lesser responsibility and checks to validate. +
*Cons:* A class should follow `Single Responsibility Principle`. Method that is not supposed to be called through `MainWindow` should be avoided.
...
The gmap mechanism is facilitated by the `GmapCommand`. `GhemeCommandParser` checks for input index and validate through `ParserUtil` and calls the relevant `GmapCommand` to search for `Person` address using Google Map.
...
The following sequence diagram shows how the theme (GmapCommand) operation works:

image::GmapCommand.png[]

The following sequence diagram shows how the theme (GmapCommandParser) operation works:

image::GmapCommandParser.png[]

==== Design Considerations
*Aspect:* Implementation of GmapCommand +
*Alternative 1 (Current choice):* Utilizes event handler and ensure that web browser won't be obstruct by person side card panel permanently by implementing extra event handler +
*Pros:* Better user experience. +
*Cons:* We could always implement library to deal with sliding person side card panel. +
