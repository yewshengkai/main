...
----
The utility class `ProcessImage` is used to process images retrieved from the Internet. It contains
...
As `SetAvatarCommand` is an `Command`, `removeImageFromStorage(...)` deletes the image once invoked. +
